...
int main(int argc, char **argv){
    ...
    if (argc == 1) {
        printf("[proNet-core]\n");
        printf("\tcommand eco interface for proNet-core\n\n");
        ...
        printf("Usage:\n");
        printf("\n[ECO]\n");
        printf("./eco -train net.txt -save rep.txt -dimensions 64 -sample_times 10 -negative_samples 5 -alpha 0.025 -reg 0.01 -threads 1\n");
    ...
    ECO *eco;
    eco = new ECO();
    eco->LoadEdgeList(network_file, 1);
    eco->LoadFieldMeta(field_file);
    eco->Init(dimensions);
    eco->Train(sample_times, negative_samples, init_alpha, reg, threads);
    eco->SaveWeights(rep_file);
