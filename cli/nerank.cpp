...
int main(int argc, char **argv){
    ...
    if (argc == 1) {
        printf("[proNet-core]\n");
        printf("\tcommand nerank interface for proNet-core\n\n");
    ...
    NERANK *nerank;
    nerank = new NERANK();
    nerank->LoadEdgeList(network_file, 1);
    nerank->LoadFieldMeta(field_file);
    nerank->Init(dimensions);
    nerank->Train(sample_times, walk_steps, init_alpha, threads);
    nerank->SaveWeights(rep_file);
