...
int main(int argc, char **argv){
    ...
    if (argc == 1) {
        ...
        printf("Usage:\n");
        printf("./gcn -train net.txt -field field.txt -save rep.txt -undirected 0 -dimensions 64 -reg 0.01 -sample_times 5 -walk_steps 5 -negative_samples 5 -alpha 0.025 -threads 1\n");
    ...
    GCN *gcn;
    gcn = new GCN();
    gcn->LoadEdgeList(network_file, undirected);
    gcn->LoadFieldMeta(field_file);
    gcn->Init(dimensions);
    gcn->Train(sample_times, walk_steps, negative_samples, reg, init_alpha, threads);
    gcn->SaveWeights(rep_file);
